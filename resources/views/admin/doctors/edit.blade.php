<x-admin-layout>
    ...
    <div class="border-l-4 border-green-300 flex justify-between m-5">
        <span class="p-4 uppercase font-semibold "> Edit doctor </span>
    ...
    <section class="flex flex-col lg:px-24 p-5 md:justify-center md:items-center" id="appointments">
        <div class="flex justify-center items-center pt-3">
            <h2 class="font-bold text-3xl text-center ">Edit <span class="text-green-400">Doctor</span></h2>
        </div>
        <form class="space-y-2 pt-5 md:w-3/4 xl:w-1/2" method="POST"
            action="{{ route('admin.doctors.update', $doctor->id) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div>
            ...
            <input type="text" name="doctor_name"
                ...
                placeholder="Doctor Name" value="{{ $doctor->doctor_name }}" />
            @error('doctor_name')
            ...
            <input type="text" name="doctor_phone"
                ...
                placeholder="Phone Number" value="{{ $doctor->doctor_phone }}" />
            @error('doctor_phone')
                <h1 class="text-sm text-red-400">{{ $message }}</h1>
            @enderror
            ...
            <label for="room_number" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Doctor room
                number<span class="text-red-500">*</span></label>
            <input type="text" name="room_number" placeholder="Room Number" value="{{ $doctor->room_number }}"
            ...
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="doctor_image">Upload
                image</label>
            <div class="">
                <img class="w-auto h-16 rounded-lg" src="{{ Storage::url($doctor->doctor_image) }}" alt="menu/img">
            </div>
