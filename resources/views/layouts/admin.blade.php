...
<head>
    ...
    <title>{{ config('app.name', 'primehealth.care') }}</title>
    {{-- <!-- Styles -->
    ...
    <div class="min-h-screen dark:bg-gray-900">
        ...
        <div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
            <div @click.away="open = false"
                class="flex flex-col flex-shrink-0 bg-green-100 text-gray-700 w-auto md:w-64 dark:text-gray-200 dark:bg-gray-800"
                x-data="{ open: false }">
                ...
                <nav :class="{ 'block': open, 'hidden': !open }"
                    class="flex-grow px-2 pb-4 pt-8 md:block md:pb-0 md:overflow-y-auto ">

                    <x-admin-nav-link :href="route('admin.doctors.index')" :active="request()->routeIs('admin.doctors.index')">
                        {{ __('Doctors') }}
                    </x-admin-nav-link>

                    <x-admin-nav-link :href="route('admin.appointments.index')" :active="request()->routeIs('admin.appointments.index')">
                        {{ __('Appointments') }}
                    </x-admin-nav-link>
                    <x-admin-nav-link :href="route('admin.news-updates.index')" :active="request()->routeIs('admin.news-updates.index')">
                        {{ __('News Updates') }}
                    </x-admin-nav-link>
                    ...
                    <div @click.away="open = false" class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            class="flex flex-row items-center w-full px-4 py-2 mt-2 text-sm font-semibold text-left bg-transparent rounded-lg dark:bg-transparent dark:focus:text-white dark:hover:text-white dark:focus:bg-green-600 dark:hover:bg-green-600 md:block hover:text-gray-900 focus:text-gray-900 hover:bg-green-200 focus:bg-green-200 focus:outline-none focus:shadow-outline">
                            <span>Admin</span>
            ...
            <main class="w-full">
                ...
                @if (session()->has('info'))
                    <div id="alert-border-1"
                        ...
                        <button type="button" id="dismissButton1"
                            class="ms-auto -mx-1.5 -my-1.5 bg-blue-50 text-blue-500 rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 hover:bg-blue-200 inline-flex items-center justify-center h-8 w-8 dark:bg-gray-800 dark:text-green-400 dark:hover:bg-gray-700"
                ...
                @if (session()->has('danger'))
                    <div id="alert-border-2"
                        ...
                        <div class="ml-3 text-sm font-medium  ">
                            <span class="font-bold">{{ session()->get('danger') }}</span>

                        </div>
                        <button type="button" id="dismissButton"
                            class="ms-auto -mx-1.5 -my-1.5 bg-red-50 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex items-center justify-center h-8 w-8 dark:bg-gray-800 dark:text-blue-400 dark:hover:bg-gray-700"
